...
一个媒体实例代表在src /输入/ vlm_internal.h的定义的结构vlm_media_instance_sys_t，中。这个结构包含一个播放列表中的索引，以及当前的输入线和输出流实例。多张的媒体实例可以存在于每个媒体。 
每个VOD服务器表示的结构vod_t。该结构包含一个指向将被加载的模块，以帮助VOD服务器，以及指针，指向一个RTSP服务器。它还包含几个函数指针，以帮助创建和删除VOD媒体类型vod_media_t。实际引用的每个vod_media_t是一个vlm_media_sys_t，如果该媒体的VOD类型。 vod_t定义在include / vlc_vod.h和中定义模块/ MISC / rtsp.c的的vod_media_t
Vlm 的主要布局：
vlm_t
    vlm_media_sys_t
        vlm_media_t
